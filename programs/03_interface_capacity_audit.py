...
log = setup('interface_capacity')
args = parser('Audit capacité interfaces').parse_args()

def audit(d):
    name = d.get('name', d['host'])
    status = run(d, 'show interfaces status', True)
    errors = run(d, 'show interfaces counters errors', True)
    rows = []
    if not isinstance(status, list):
        return [{'device': name, 'error': 'TextFSM indisponible', 'raw': str(status)}]
    errmap = {str(x.get('port') or x.get('interface')): x for x in errors} if isinstance(errors, list) else {}
    for x in status:
        port = str(x.get('port') or x.get('interface') or '')
        state = str(x.get('status', '')).lower()
        desc = x.get('name') or x.get('description') or ''
        e = errmap.get(port, {})
...
rows = [x for g in groups for x in (g if isinstance(g, list) else [g])]
wb = Workbook()
ws = wb.active
ws.title = 'Interfaces'
...
path = OUTPUT / f'interface_capacity_{stamp()}.xlsx'
wb.save(path)
