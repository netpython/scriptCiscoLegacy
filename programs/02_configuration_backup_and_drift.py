...
log = setup('backup_drift')
p = parser('Backup et dérive de configuration')
p.add_argument('--baseline', default='baselines')
...
results = parallel(devices(args.inventory), audit, args.workers)
folder = OUTPUT / f'drift_{stamp()}'
folder.mkdir()
for r in results:
    if r.get('diff'):
        (folder / f"{r['device']}.diff").write_text(r.pop('diff'), encoding='utf-8')
(folder / 'summary.json').write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding='utf-8')
log.info('Rapport: %s', folder)
